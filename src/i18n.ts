...
const UI = {
  ja: {
    ...
    showLabels: 'ラベルを表示',
    debugMode: 'デバッグモード',
    debugLegend: '青線=人物の向き、緑線=カメラの視線方向',
    ...
    helpJSON: '「JSON保存」でシーン全体を保存。「JSON読込」で再現できます。',
    close: '閉じる',
  },
  en: {
    title: '3D Simulation',
    ...
    debugLegend: 'Blue=character facing, green=camera view direction',
    ...
    helpJSON: 'Use Save JSON to save the whole scene; Load JSON reproduces it.',
    close: 'Close',
  },
  sv: {
    title: '3D-simulering',
    ...
    debugLegend: 'Blå=personens riktning, grön=kamerans riktning',
    ...
    helpJSON: 'Spara JSON sparar hela scenen; Läs in JSON återskapar den.',
    close: 'Stäng',
  },
} as const
...
export const KIND_TEXT: Record<Lang, Record<KindId, KindText>> = {
  ja: {
    ...
    family: { label: '家族', sub: 'familj・family', labelText: '家族' },
    school: { label: '学校' },
    block_tall: { label: '縦ブロック', sub: '校門・ドア' },
    block_flat: { label: '横ブロック', sub: '机' },
    present: { label: 'プレゼント' },
  ...
  en: {
    ...
    family: { label: 'Family', sub: 'familj・family', labelText: 'Family' },
    school: { label: 'School' },
    block_tall: { label: 'Tall block', sub: 'gate・door' },
    block_flat: { label: 'Flat block', sub: 'desk' },
  ...
  sv: {
    ...
    school: { label: 'Skola' },
    block_tall: { label: 'Stående block', sub: 'grind・dörr' },
    block_flat: { label: 'Liggande block', sub: 'skrivbord' },
    present: { label: 'Present' },
